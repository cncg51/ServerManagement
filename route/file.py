                ...
                if not os.path.isdir(i):
                    if os.path.islink(i):
                        ...
                        file_.append({
                            ...
                            i,
                            'fileSize':
                            ('%.2f' % (os.stat(i).st_size / 1024)) + 'k',
                            'fileOnlyName':
                            os.path.split(i)[1] + '-->' + fileLinkPath,
                            'fileMODTime':
                            time.strftime("%Y-%m-%d %H:%M:%S",
                                          time.localtime(os.stat(i).st_mtime)),
                            'power':
                            oct(os.stat(i).st_mode)[-3:],
                            'fileType':
                            'file'
                        })
                    else:
...
def RenameFile():
    try:
        ...
        if newFileName in os.listdir(filePath):
            ...
            os.rename(
                os.path.join(filePath, oldFileName),
...
def zip_(fileList, zipPath):
    try:
        ...
        f = zipfile.ZipFile(
            os.path.join(zipPath, zipName) + '.zip', 'w', zipfile.ZIP_DEFLATED)
        for i in fileList:
            if os.path.isdir(i):
