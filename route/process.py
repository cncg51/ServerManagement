...
ps = [
    ...
    'WPA_SUPPLICANT', 'LVMETAD', 'AUDITD', 'MASTER', 'DBUS-DAEMON', 'TAPDISK',
    'SSHD', 'INIT', 'KSOFTIRQD', 'KWORKER', 'KMPATHD', 'KMPATH_HANDLERD',
    'PYTHON', 'KDMFLUSH', 'BIOSET', 'CROND', 'KTHREADD', 'MIGRATION',
    'RCU_SCHED', 'KJOURNALD', 'IPTABLES', 'SYSTEMD', 'NETWORK', 'DHCLIENT',
    'SYSTEMD-JOURNALD', 'NETWORKMANAGER', 'SYSTEMD-LOGIND', 'SYSTEMD-UDEVD',
    'POLKITD', 'TUNED', 'RSYSLOGD', 'BASH', 'YDSERVICE', 'SYSTEMD'
]
...
def GetNetWorkList():
    ...
    while networkList != []:
        jsonName = os.path.join('temp',
                                str(time.time() + random.random()) + '.json')
    ...
    response = make_response(
        send_from_directory(
            os.path.split(t)[0], os.path.split(t)[1], as_attachment=True))
    response.headers["Content-Disposition"] = "attachment; filename={}".format(
        (t + '.zip').encode().decode('latin-1'))
    return response
...
def GetProcessList():
    ...
    for pid in Pids:
        try:
            ...
            tmp['memory_percent'] = str(
                round(p.memory_percent(), 3)) + '%'  #进程占用的内存比例
            ...
            continue
    processList = sorted(
        processList, key=lambda x: x['memory_percent'], reverse=True)
...
def ProcessDetails():
    try:
        ...
        ProcessDict = {
            ...
            'ProcessMemory':
            str(round(p.memory_percent(), 3)) + '%',
            'ProcessThreads':
            p.num_threads(),
            'ProcessCPU':
            str(p.cpu_percent(0.2)) + '%',
            'ProcessUser':
            p.username(),
            'ProcessReadCount':
            proIO.read_count,
