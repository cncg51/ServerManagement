...
class sqlClass(object):
    ...
    def createSystemInfo(self):
        ...
        if logDB in os.listdir(sqlpath):
            self.con = sqlite3.connect(
                os.path.join(sqlpath, logDB), check_same_thread=False)
        else:
            self.con = sqlite3.connect(
                os.path.join(sqlpath, logDB), check_same_thread=False)
    ...
    def insertInfo(self, info):
        ...
        self.con.execute("INSERT INTO SYSTEMINFO (INFO,TIM) VALUES (?,?)",
                         (info, self.getTime()))
        self.con.commit()
    ...
    def selectInfo(self, day):
        try:
            date = time.strftime(
                '%Y-%m-%d %H:%M:%S',
                time.localtime(time.time() - (int(day) * 86400)))
            resultData = self.con.execute(
                'SELECT * FROM SYSTEMINFO WHERE TIM > (?)',
                (date, )).fetchall()
    ...
    def insertRemoteHost(self,
        ...
        try:
            self.con.execute(
                "INSERT INTO RemoteHost (IP,PORT,CTYPE,USERNAME,PWD,PKPATH,GROUPS,NOTE,CARETETIME,ROOTPWD) VALUES (?,?,?,?,?,?,?,?,?,?)",
                (IP, PORT, CTYPE, USERNAME, PWD, PKPATH, GROUPS, NOTE,
                 self.getTime(), ROOTPWD))
            self.con.commit()
        except Exception as e:
            return [False, str(e)]
        else:
    ...
    def selectRemoteHost(self):
        try:
            resultData = self.con.execute(
                'SELECT IP,PORT,USERNAME,GROUPS,NOTE,CARETETIME FROM RemoteHost'
            ).fetchall()
            result = [True, resultData]
        except Exception as e:
            result = [False, str(e)]
    ...
    def selectRemoteHostForIP(self, IP):
        try:
            resultData = self.con.execute(
                'SELECT IP,PORT,USERNAME,PWD,ROOTPWD FROM RemoteHost WHERE IP = (?)',
                (IP, )).fetchall()[0]
            result = [True, resultData]
        except Exception as e:
            result = [False, str(e)]
